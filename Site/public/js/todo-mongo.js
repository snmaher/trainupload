 ...
 savelocation: function(itemid, latitude, longitude)
 {
  ...
  bb.model.Items = Backbone.Collection.extend(_.extend({    
   ...
   // localStorage: new Store("items"),
    ...
    additem: function(textentered1, textentered2, textentered3) {
      var self = this
	  
      var item = new bb.model.Item({
        destination: textentered1, done:false, listid: app.model.state.get('currentlist'), route: textentered2, time: textentered3
      })
      self.add(item)
      self.count++
      item.save() 
   ...
   bb.view.Head = Backbone.View.extend(_.extend({    
	...
	addselected : function(){ 
	  ...
	  newlistselected : function(){
		 var self = this
		 self.elem.newitem.hide() 
		 self.elem.newlist.slideDown()
		 self.elem.listtext2.val("")
		 $("#listcontent").css({
         top: 190 })
	  ...
	  saveselected : function(){ 
		...
		var textentered1 = self.elem.text1.val()
		var textentered2 = self.elem.text2.val()
		var textentered3 = self.elem.text3.val()
		
		self.items.additem(textentered1, textentered2, textentered3)
	...
	initialize: function( items, lists, preferences ) {
      ...
      self.elem = {
		...
		text1: self.$el.find('div #text1'),
		text2: self.$el.find('div #text2'),
		text3: self.$el.find('div #text3'),
